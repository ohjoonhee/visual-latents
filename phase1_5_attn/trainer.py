"""Phase 1.5 trainer: Monet's vendored modified Qwen2.5-VL transformer + Phase 1 LVR loss + Phase 1 data.

Goal: isolate whether the architectural mechanism (Monet's `latent_mode=True`
recurrent latent forward + 4D attention rules) is what produces distributed
encoder-grounded latents in Monet stage 2 — independent of the loss form.

Recipe (everything else identical to Phase 1 run 1):
- backbone: Monet vendored Qwen2.5-VL-3B-Instruct (sys.modules patch)
- prompt format: ...<|vision_start|><|image_pad|>×N<|vision_end|>{question}...
                 <|im_start|>assistant<abs_vis_token><abs_vis_token_pad>×8</abs_vis_token>{answer}<|im_end|>
- loss: L_NTP + 1.0 * F.mse_loss(h, v_roi); h = outputs.latent_embeds[0]; mean over (1,K,D)
- LLM full FT, vision tower + projector frozen
- AdamW lr=1e-5, warmup 100 cosine, eff bsz=4 (bsz=1 × grad_accum=4), 1000 steps
- data: ohjoonhee/visual-cot-50k-poc 5k train, seed=0
"""
...
def _build_processor(base: str):
    ...
    # Add Monet special tokens (idempotent).
    for tok in [
        "<abs_vis_token_pad>",
        "<abs_vis_token>",
        "</abs_vis_token>",
...
def _build_prompt(question: str, answer: str, K: int, n_image_tokens: int) -> tuple[str, str]:
    """Phase 1.5 prompt format using Monet's slot tokens."""
...
def lvr_step_monet(
    ...
    """Single Phase 1.5 fwd/bwd (no opt.step). Returns dict of stats + loss tensor."""
    ...
    # Sanity: prefix is a strict prefix of full.
    ...
    # Locate the K latent-pad positions (these are <abs_vis_token_pad> tokens).
    pad_positions = (full_ids[0] == latent_pad_id).nonzero(as_tuple=False).squeeze(-1)
    if pad_positions.numel() < K:
        raise RuntimeError(
            f"Expected ≥{K} latent-pad positions, got {pad_positions.numel()}"
        )
    slot_positions = pad_positions[:K]

    # Locate image_pad positions (post-merger image tokens).
    img_pad_positions = (full_ids[0] == image_token_id).nonzero(as_tuple=False).squeeze(-1)
    if img_pad_positions.numel() != n_image_tokens:
        raise RuntimeError(
            f"Expected {n_image_tokens} image_pad positions, got {img_pad_positions.numel()}"
        )

    # Labels: only score answer tokens. Mask special tokens (latent_pad, latent_start/end,
    # image_pad, vision_start/end) since those are structural.
    L = full_ids.shape[1]
    ...
    # This runs Monet's recurrent latent generation: each <abs_vis_token_pad>
    # position is filled by the previous-token's hidden state, then forwarded.
    # Returns latent_embeds (the K hidden states at slot positions) AND
    # ce_patch_pos/ce_patch_vec for the next-stage CE forward.
    #
    # Following upstream trainer.py:160-163, latent forward must NOT use
    # gradient checkpointing (uses use_cache=True).
    ...
    # h: [K, D] last-layer hidden state at the K slot positions, grad-enabled.
    # `output_latent_embeds=True` returns a `.detach()`-ed version which would zero LVR
    # gradients; instead use `outputs.hidden_states[b]` which is shape
    # [num_layers, K, D] in latent_mode and keeps grad in train mode.
    ...
    # --- 5. CE forward (latent_mode=False) with spliced latent embeds ---
    ...
    # LVR loss: MSE(h, v_roi), mean over (1, K, D) — matches Phase 1 run 1.
...
def main():
    ...
    ap.add_argument("--smoke", action="store_true", help="10-step gradient probe")
    args = ap.parse_args()
    ...
    base = cfg["base_model"]
    print(f"[load] {base} (Monet-vendored class)", flush=True)
    ...
    # Resize for new tokens (Monet's tokens not in base vocab).
