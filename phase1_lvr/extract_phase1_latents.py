"""Extract Phase 1 latents from a trained checkpoint on the held-out eval split.

For each held-out (image, question, answer, bbox), build the same prompt as
training (with K=8 latent slots in the assistant header), run a forward pass,
and save the K hidden-state vectors at the slot positions. These are the
analogue of Monet's stage-2 latents for the Phase 0 ablation suite.

Output: phase1_lvr/results/latents/latent_<sid>.pt with keys
  - latent: Tensor [K, H]   (last-layer hidden states at K latent slots)
  - question, answer, bbox, sample_id
"""
