...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"[vl.train] loaded config: name={cfg.name} cell={cfg.cell} variant={cfg.trainer.variant}")
    ...
    if cfg.trainer.variant == "A":
        # from vl.trainers.sft_anchor import SFTAnchorTrainer  # noqa: ERA001
        raise NotImplementedError("Variant A trainer not yet implemented (v0.1.0 scaffold).")
    elif cfg.trainer.variant == "B":
        if cfg.vlpo is None:
        ...
        raise ValueError(f"Unknown variant: {cfg.trainer.variant!r}")
