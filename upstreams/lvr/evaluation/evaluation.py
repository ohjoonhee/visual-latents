...
# [LVR-repro edit] Removed Oracle-Cloud (OCI) checkpoint handler import + hardcoded creds (unused for eval).
...
CHKPT_PATHS = ["/mnt/ssd/Projects/visual-latents/checkpoints/lvr-7b-released"]  # [LVR-repro edit]
...
def load_model_and_processor(chkpt_pth):
    ...
    model = QwenWithLVR.from_pretrained(
        ...
        torch_dtype="auto",
        attn_implementation="sdpa",  # [LVR-repro edit] flash_attention_2 -> sdpa (avoid flash-attn build; greedy decode identical)
        device_map="auto",)
...
def load_vstar_dataset(gen_w_head,run_name,decoding_strategy):
    ...
    image_dir = "/mnt/ssd/Projects/visual-latents/lvr_eval/data/vstar_bench"

    if gen_w_head:
        out_dir = os.path.join("/mnt/ssd/Projects/visual-latents/lvr_eval/data/vstar_bench/results",f"decoding_by_{decoding_strategy}","GenWHead"+run_name)

    else:
        out_dir = os.path.join("/mnt/ssd/Projects/visual-latents/lvr_eval/data/vstar_bench/results",f"decoding_by_{decoding_strategy}",run_name)
...
def load_mmvp_dataset(gen_w_head,run_name,decoding_strategy):
    ds_name = "mmvp"
    csv_file = "/mnt/ssd/Projects/visual-latents/lvr_eval/data/MMVP/Questions.csv"
    image_dir = "/mnt/ssd/Projects/visual-latents/lvr_eval/data/MMVP/MMVP_Images"
    ...
    if gen_w_head:
        out_dir = os.path.join("/mnt/ssd/Projects/visual-latents/lvr_eval/data/MMVP/results",f"decoding_by_{decoding_strategy}","GenWHead"+run_name)

    else:
        out_dir = os.path.join("/mnt/ssd/Projects/visual-latents/lvr_eval/data/MMVP/results",f"decoding_by_{decoding_strategy}",run_name)
...
def load_blink_dataset(gen_w_head,run_name,decoding_strategy):
    ...
    if gen_w_head:
        out_dir = os.path.join("/mnt/ssd/Projects/visual-latents/lvr_eval/data/blink/results",f"decoding_by_{decoding_strategy}","GenWHead"+run_name)

    else:
        out_dir = os.path.join("/mnt/ssd/Projects/visual-latents/lvr_eval/data/blink/results",f"decoding_by_{decoding_strategy}",run_name)
...
def main():
    ...
    for checkpoint_dir in CHKPT_PATHS:
        ...
        print("\n" + "="*128 + "\nEvaluating model:\n" +  f"{checkpoint_dir} \n"+"="*128 + "\n")
        for bench_name in ["vstar", "MMVP", "blink"]:  # [LVR-repro edit] headline/validated first, BLINK (auto-download) last
