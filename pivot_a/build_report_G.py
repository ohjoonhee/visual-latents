"""Build pivot_a/REPORT_G.md — Experiment G (K=4 + 2× steps stacking probe).

Decoupled from build_report_F.py (which keeps F's 4-column comparison stable)
to add a 5th column for G alongside D2/E/F/Monet.

Reads:
  - D2 (K=8) at pivot_a/results/vicreg_lambda2 — for comparison
  - E  (K=8 @ 2k) at pivot_a/results/vicreg_lambda2_2k — for comparison
  - F  (K=4 @ 1k) at pivot_a/results/vicreg_K4 — for comparison
  - G  (K=4 @ 2k) at pivot_a/results/vicreg_K4_2k

Produces REPORT_G.md with:
  - 5-column comparison: D2 / E / F / G / Monet stage 2
...
RES_G       = ROOT / "results" / "vicreg_K4_2k"
...
def main():
    ...
    g_k4_2k = _load_variant(RES_G,       "pivot_a_vicreg_K4_2k_self",       K_default=4)

    table = [
        "| metric | D2 (K=8) | E (K=8 @ 2k) | F (K=4 @ 1k) | **G (K=4 @ 2k)** | Monet stage 2 |",
        "|---|---:|---:|---:|---:|---:|",
        f"| K | {lambda2['K']} | {e_2k['K']} | {f_k4['K']} | **{g_k4_2k['K']}** | 8 |",
        f"| compression_ratio (≥ 0.4) | {fmt(lambda2['cr_self'])} | {fmt(e_2k['cr_self'])} | {fmt(f_k4['cr_self'])} | **{fmt(g_k4_2k['cr_self'])}** | 2.7 |",
        f"| **mean off-diag cos (≤ 0.55)** | {fmt(lambda2['mean_cos'])} | {fmt(e_2k['mean_cos'])} | {fmt(f_k4['mean_cos'])} | **{fmt(g_k4_2k['mean_cos'])}** | 0.38 |",
        f"| n_helpful | {lambda2['nh_self']}/{lambda2['K']} | {e_2k['nh_self']}/{e_2k['K']} | {f_k4['nh_self']}/{f_k4['K']} | **{g_k4_2k['nh_self']}/{g_k4_2k['K']}** | 4/8 |",
        f"| qwen_base utility (> 0) | {fmt(lambda2['util_base'])} | {fmt(e_2k['util_base'])} | {fmt(f_k4['util_base'])} | **{fmt(g_k4_2k['util_base'])}** | +2.7 |",
        f"| utility (self reader) | {fmt(lambda2['util_self'])} | {fmt(e_2k['util_self'])} | {fmt(f_k4['util_self'])} | **{fmt(g_k4_2k['util_self'])}** | +2.7 |",
        f"| v_roi off-diag cos | {fmt(lambda2['v_off'])} | {fmt(e_2k['v_off'])} | {fmt(f_k4['v_off'])} | **{fmt(g_k4_2k['v_off'])}** | n/a |",
    ]
...
"""
    ...
    # G verdict logic — does stacking close the gap further than F alone?
    g_geom_hit = (g_k4_2k['mean_cos'] is not None and g_k4_2k['mean_cos'] <= 0.55)
    g_helpful_ratio = (g_k4_2k['nh_self'] / g_k4_2k['K']) if g_k4_2k['K'] else 0.0
    f_helpful_ratio = (f_k4['nh_self'] / f_k4['K']) if f_k4['K'] else 0.0
    g_util_base = g_k4_2k['util_base'] if g_k4_2k['util_base'] is not None else 0.0
    f_util_base = f_k4['util_base'] if f_k4['util_base'] is not None else 0.0

    if not g_k4_2k['has_data']:
        verdict_line = "G (K=4 @ 2k): training/ablation not yet complete."
    elif g_geom_hit and g_helpful_ratio >= 0.75 and g_util_base >= 0.20:
        verdict_line = (
            f"G (K=4 @ 2k) **STACKS strongly**: helpful ratio {g_helpful_ratio:.2f} ≥ 0.75 "
            f"(F: {f_helpful_ratio:.2f}), qwen_base utility = {fmt(g_util_base)} ≥ +0.20 "
            f"(F: {fmt(f_util_base)}). K=4 + 2× steps closes more of the semantic gap; "
            f"adopt for cluster Phase 3."
        )
    elif g_geom_hit and g_util_base > f_util_base + 0.02:
        verdict_line = (
            f"G (K=4 @ 2k) **partially stacks**: utility {fmt(g_util_base)} > F's {fmt(f_util_base)} "
            f"(helpful ratio {g_helpful_ratio:.2f} vs F {f_helpful_ratio:.2f}). "
            f"Longer training on top of K=4 still helps, but not enough to clear the n_helpful=75% bar."
        )
    elif g_geom_hit and abs(g_util_base - f_util_base) < 0.02:
        verdict_line = (
            f"G (K=4 @ 2k) **does NOT stack**: utility {fmt(g_util_base)} ~ F's {fmt(f_util_base)}, "
            f"helpful ratio {g_helpful_ratio:.2f} (F: {f_helpful_ratio:.2f}). "
            f"K=4 already saturates at 1000 steps; the residual gap is positional/structural."
        )
    elif not g_geom_hit:
        verdict_line = (
            f"G (K=4 @ 2k) **breaks geometry**: mean off-diag cos = {fmt(g_k4_2k['mean_cos'])} > 0.55. "
            f"Doubling steps with VICReg λ_reg=2.0 may collapse representation at K=4."
        )
    else:
        verdict_line = (
            f"G (K=4 @ 2k) **mixed**: helpful ratio {g_helpful_ratio:.2f} (F: {f_helpful_ratio:.2f}), "
            f"utility {fmt(g_util_base)} (F: {fmt(f_util_base)}), mean cos {fmt(g_k4_2k['mean_cos'])}."
        )

    body = f"""# Pivot A — Experiment G (K=4 + 2× steps stacking probe)
...
{acc_block(lambda2, "D2 — VICReg λ_reg=2.0, K=8 @ 1k (mean-MSE LVR)")}

{acc_block(e_2k,    "E  — D2 recipe @ 2000 steps, K=8")}

{acc_block(f_k4,    "F  — D2 recipe @ K=4, 1000 steps")}

{acc_block(g_k4_2k, "G  — D2 recipe @ K=4 + 2000 steps (this experiment)")}

{variant_section(lambda2, "D2 (K=8 @ 1k)")}

{variant_section(e_2k,    "E (K=8 @ 2k)")}

{variant_section(f_k4,    "F (K=4 @ 1k)")}

{variant_section(g_k4_2k, "G (K=4 @ 2k)")}
...
"""
    ...
    out_path = ROOT / "REPORT_G.md"
    out_path.write_text(body)
    print(f"Wrote {out_path}")
    print(f"\nG (K=4 @ 2k): {g_k4_2k['overall']} ({g_k4_2k['n_pass']}/4)  mean_cos={fmt(g_k4_2k['mean_cos'])}  "
          f"n_helpful={g_k4_2k['nh_self']}/{g_k4_2k['K']}  util_base={fmt(g_k4_2k['util_base'])}")
    print(f"F (K=4 @ 1k): {f_k4['overall']} ({f_k4['n_pass']}/4)  mean_cos={fmt(f_k4['mean_cos'])}  "
          f"n_helpful={f_k4['nh_self']}/{f_k4['K']}  util_base={fmt(f_k4['util_base'])}")
    print(f"E (K=8 @ 2k): {e_2k['overall']} ({e_2k['n_pass']}/4)  mean_cos={fmt(e_2k['mean_cos'])}  "
          f"n_helpful={e_2k['nh_self']}/{e_2k['K']}  util_base={fmt(e_2k['util_base'])}")
    print(f"D2 (K=8 @ 1k): {lambda2['overall']} ({lambda2['n_pass']}/4)  mean_cos={fmt(lambda2['mean_cos'])}  "
