"""Pivot A ablation = Phase 1 ablation (no Monet patches).

Two readers:
  - <self_name> (e.g. "pivot_a_cos_self"): the trained Pivot A checkpoint, frozen
  - qwen_base: fresh Qwen/Qwen2.5-VL-3B-Instruct, frozen — transfer test

Same prompt template (<|image_pad|>) as Phase 1.

Outputs:
  - results/<run>/ablation_results.jsonl
  - results/<run>/ablation_h_stats.jsonl  (also reports v_roi_avg_off_diag_cosine)
"""
...
def main():
    ...
    ap.add_argument("--self_ckpt", type=str, required=True)
    ap.add_argument("--self_name", type=str, default="pivot_a_self")
    ...
    proc1 = AutoProcessor.from_pretrained(args.self_ckpt, use_fast=True, trust_remote_code=True)
    ...
    run_reader(reader_name=args.self_name, model=m1, tokenizer=proc1.tokenizer, image_token_id=img_id1,
